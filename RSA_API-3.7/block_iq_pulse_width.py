"""
#### NEW RSA_API VERSION ####
Tektronix RSA306 API V2: Block IQ with Pulse Width Calculator
...
def pulse_width_finder(data, thresh, startIndex):
    ...
    for j in xrange(risingIndex, endIndex):
        if data[j] >= dPoint:
            saved = data[j]
        else:
          if saved >= dPoint:
...
def main():
    ...
    try:
        del(pulseWidth[0])
        del(pulseWidth[-1])
        for i in xrange(len(pulseWidth)):
    ...
    for i in xrange(len(pwRisingIndices)-1):
